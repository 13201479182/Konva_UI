...
export abstract class Node<Config extends NodeConfig = NodeConfig> {
     ...
     */
    findAncestors(selector: string | Function, includeSelf?: boolean, stopNode?: Node) {
        var res: Array<Node> = [];
     ...
     */
    findAncestor(selector: string | Function, includeSelf?: boolean, stopNode?: Container) {
     ...
     */
    isClientRectOnScreen(margin: { x: number; y: number } = { x: 0, y: 0 }): boolean {
        const stage = this.getStage();
        if (!stage) {
            return false;
        }
